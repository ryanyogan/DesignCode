...
struct RadialLayoutView: View {
    var icons = [
        "calendar", "message", "figure.walk", "music.note"
    ]
    ...
    var body: some View {
        ...
        ZStack {
            ...
            Circle().fill(.white).frame(width: 3)
        }
        .onAppear() {
            hour = 360
            minute = 360
...
struct CustomLayout: Layout {
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) {
        for (index, subview) in subviews.enumerated() {
            // Position
            var point = CGPoint(x: 50 * index, y: 20 * index).applying(CGAffineTransform(rotationAngle: CGFloat(6 * index + 6)))
